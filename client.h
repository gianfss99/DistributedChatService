...
void IClient::run()
{
    int conn = contactCoord();
    if(conn < 0){
        std::cout << "connection failed to Coordinator: " << conn << std::endl;
        exit(1);
    }
    int ret = connectTo();
    if (ret < 0) {
        std::cout << "connection failed to Server: " << ret << std::endl;
        exit(1);
    }
    displayTitle();
    while (1) {
